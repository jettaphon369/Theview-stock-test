...
const BUILD_VERSION='34.12.11-TEST';
...
function setupAppShell(){
  ...
  if(hero && !document.getElementById('headerMenuBtn')){
    ...
    const positionHeaderMenu=()=>{
      ...
      const safeTop=(vv ? vv.offsetTop : 0) + 10;
      const side=Math.max(10, Math.round(viewportW - rect.right));
      panel.style.right=`${side}px`;
      panel.style.maxHeight='none';
      const panelH=panel.offsetHeight || 0;
      let top=Math.round(rect.bottom + 8);
      const minTop=Math.round(safeTop);
      const maxTop=Math.max(minTop, Math.round(safeTop + viewportH - panelH - 12));
